...
#define PROID            "28z738h3zx"

#define ACCESS_KEY        "6z3fV9Uxwy8l29Eth2Evs82BM/klqR8frVG5yVVnJWE="

#define DEVICE_NAME        "template"

#define MANUAL_OVERRIDE_DURATION_SECONDS 300 //手动模式持续时间
#define MANUAL_OVERRIDE_TIMEOUT_VALUE (MANUAL_OVERRIDE_DURATION_SECONDS * 100)
...
extern u8 temp,humi,temp_value,humi_value;
extern u16 soil_adc_raw, co;
extern double rain_mm_calc;
extern u8 g_motor_status;
extern u8 g_MOSFET_status;
unsigned char OneNet_FillBuf(char *buf)
{
    ...
    memset(text, 0, sizeof(text));
    sprintf(text, "\"Co\":{\"value\":%d},", co);
    strcat(buf, text);

    memset(text, 0, sizeof(text));
    sprintf(text, "\"temp_value\":{\"value\":%d},", temp_value);
    strcat(buf, text);
    
    memset(text, 0, sizeof(text));
    sprintf(text, "\"humi_value\":{\"value\":%d},", humi_value);
    strcat(buf, text);
    
    memset(text, 0, sizeof(text));
    sprintf(text, "\"Soil_value\":{\"value\":%d},", soil_adc_raw);
    strcat(buf, text);
    /*
    memset(text, 0, sizeof(text));
    sprintf(text, "\"Rain_value\":{\"value\":%d},", (uint16_t)rain_mm_calc);
    strcat(buf, text);
    */
    memset(text, 0, sizeof(text));
    ...
    memset(text, 0, sizeof(text));
    sprintf(text, "\"MOSFET\":{\"value\":%s},", g_MOSFET_status? "true" : "false");
    strcat(buf, text);
    
    memset(text, 0, sizeof(text));
    sprintf(text, "\"Motor\":{\"value\":%s}", g_motor_status? "true" : "false");
    strcat(buf, text);
    
    strcat(buf, "}}");
    UsartPrintf(USART_DEBUG, "JSON Length: %d, Content: %s\r\n", strlen(buf), buf);
    return strlen(buf);
...
extern uint32_t g_manual_light_override_timeout_ticks;
extern u8 g_motor_manual_override;
extern uint32_t g_manual_motor_override_timeout_ticks;
extern u8 g_MOSFET_manual_override;
extern uint32_t g_manual_MOSFET_override_timeout_ticks;
void OneNet_RevPro(unsigned char *cmd)
{
    ...
    cJSON *raw_json, *params_json, *led_json, *motor_json, *MOSFET_json, *temp_value_json, *humi_value_json;
    ...
    switch(type)
    {
        case MQTT_PKT_PUBLISH:                                                                //接收的Publish消息
            ...
            if(result == 0)
            {
                ...
                motor_json = cJSON_GetObjectItem(params_json,"Motor");
                MOSFET_json = cJSON_GetObjectItem(params_json,"MOSFET");
                temp_value_json = cJSON_GetObjectItem(params_json,"temp_value");
                humi_value_json = cJSON_GetObjectItem(params_json,"humi_value");
                if(led_json != NULL)
                {
                    g_light_manual_override = 1; // 进入手动模式
                    g_manual_light_override_timeout_ticks = 100; // 重置超时
                    if(led_json->type == cJSON_True) Led_Set(LED_ON);
                    else Led_Set(LED_OFF);
                }
                if(motor_json != NULL)
                {
                    g_motor_manual_override = 1; // 进入手动模式
                    g_manual_motor_override_timeout_ticks = 100; // 重置超时
                    if(motor_json->type == cJSON_True) g_motor_status=1;
                    else g_motor_status=0;
                }
                if(MOSFET_json != NULL)
                {
                    g_MOSFET_manual_override = 1; // 进入手动模式
                    g_manual_MOSFET_override_timeout_ticks = 100; // 重置超时
                    if(MOSFET_json->type == cJSON_True) g_MOSFET_status=1;
                    else g_MOSFET_status=0;
                }

                if(temp_value_json != NULL)
                {
                    temp_value = temp_value_json->valueint;
                }
                if(humi_value_json != NULL)
                {
                    humi_value = humi_value_json->valueint;
                }
